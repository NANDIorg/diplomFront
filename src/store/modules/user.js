...
export const actions = {
    async registerUser({commit}, newUser){
      console.log('http://'+state.domen+'/api/user/reg');
      console.log(newUser);
        await axios.post('http://localhost:13373/api/user/reg', newUser)
        .then(res => {
    ...
    async authUser({commit}, newUser){
        await axios.post('http://'+state.domen+'/api/user/auth', newUser)
        .then(res => {
            commit('SET_TOKEN', res.data.token)
            commit('SET_MONEY', res.data.money)
            commit('SET_LOGIN', newUser.login)
    ...
    async getServerMoney({commit}){
        await axios.get('http://'+state.domen+'/api/user/getMoney', {headers: {'token': localStorage.token}})
        .then(res => {
            commit('SET_MONEY', res.data.money)
            console.log(res)
        })
    ...
    async pushMoney({commit}, moneyAndToken){
        ...
        await axios.post('http://'+state.domen+'/api/user/pushMoney', moneyAndToken)
        .then(res => {
            console.log(res)
        })
        .catch(err => {
    ...
    async createDispute({commit}, teams){
        console.log(teams)
        await axios.post('http://'+state.domen+'/api/disput/createDisput', teams)
        .then(res => {
            console.log(res)
        })
        .catch(err => {
